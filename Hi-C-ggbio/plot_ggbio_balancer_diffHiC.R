...
prepare_regions_of_interest_as_list_of_lists <- function()
{
    ...
    regions <- list()
    for (i in seq_along(diffHiC_both_DE_clusters))
    {
        m <- round((start(diffHiC_both_DE_clusters)[i] + end(diffHiC_both_DE_clusters)[i]) / 2)
        regions[[length(regions)+1]] <-
            GRanges(seqnames(diffHiC_both_DE_clusters)[i], IRanges(m - par$span / 2 + 1, m + par$span / 2), seqinfo = Seqinfo(sqlvls, genome="dm6"))
