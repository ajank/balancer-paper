...
source("regions.R") # regions, tad.lines.dm6, tad.lines.dm6bal3
...
for (i in seq_along(regions))
{
    ...
    if(!is.list(grs) || length(grs)<1) { 
        ...
        # factor 1.1 is used to make the plots match the default "expand = c(0, 0.05)" used previously
        span = 500e3 * 1.1
        my.file = paste0(as.character(seqnames(grs[[1]])), "_", 
                          round(start(grs[[1]])/1000000,2), "Mb_span",
                          round(span/1000), "kb.pdf")
        message("---", my.file, "---")
        plts <- list()

        cairo_pdf(tmp.file, width = 10*2, height = 7*2, onefile = T)
        for (wh in grs) {
            start(wh) = start(wh) - span/2
            end(wh) = end(wh) + span/2
            my.genome <- genome(wh)[1]
            for (sample in c("embryo"))
            {
                if (my.genome == "dm6") {
                    my.vlines = start(subsetByOverlaps(tad.lines.dm6, wh))
                    plts <- c(plts, evalq(main_plot(wh, sample, my.genome, add_lines = my.vlines), env.dm6))
                } else {
                    my.vlines = start(subsetByOverlaps(tad.lines.dm6bal3, wh))
                    plts <- c(plts, evalq(main_plot(wh, sample, my.genome, add_lines = my.vlines), env.dm6bal3))
                }
            }
        }
        dev.off()

        cairo_pdf(my.file, width = 10*2, height = 7*2, onefile = T)
        for (p in plts)
        {
            grid.newpage()
