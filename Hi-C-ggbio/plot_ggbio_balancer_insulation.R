...
source("main_balancer_insulation.R",      local=env.dm6)
...
source("main_balancer_insulation.R",      local=env.dm6bal3)
...
par <- list(span = 500e3, width = 10, height = 5)
# par <- list(span = 1000e3, width = 10, height = 5)
# par <- list(span = 300e3, width = 5*2, height = 6*2)

for (i in seq_along(regions))
{
    grs <- regions[[i]]$grs
    prefix <- regions[[i]]$prefix
    # if (!(prefix %in% c("BR_chr2_6", "BR_chr3_8"))) next
    if (!grepl("^M2", prefix)) next
    # span <- regions[[i]]$span
    ...
    if(!is.list(grs) || length(grs)<1) { 
        ...
        for (wh in grs) {
            start(wh) = start(wh) - par$span/2
            end(wh) = end(wh) + par$span/2
