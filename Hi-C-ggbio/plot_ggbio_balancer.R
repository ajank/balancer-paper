...
source("main_balancer.R",      local=env.dm6)
...
source("main_balancer.R",      local=env.dm6bal3)
...
par <- list(span = 500e3, width = 8*2, height = 6*2)
...
for (i in seq_along(regions))
{
    ...
    # if (prefix != "BR_chr3_8") next
    # if (prefix != "INV2_BAL") next
    # span <- regions[[i]]$span
